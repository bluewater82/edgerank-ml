...
Functional general XGBoost model that performs market predictions based
on many different tickers.
...
def evaluate_binary_threshold(model, X_test, y_test, threshold: float = 0.55):
    ...
    print(f"Predicted UP rate: {y_pred.mean():.4f}")
...
def run_pipeline():
    ...
    evaluate_binary_threshold(model, X_test, y_test, threshold=0.65)
    ...
    inspect_feature_importance(model)
